const menuData_en = [

    {
        name: "Abalone Pasta",
        url: "https://www.youtube.com/watch?v=-AjoAl9SeWw",
        uploader: "",
        tip: "A luxurious pasta dish featuring the chewy texture and rich seafood flavor of abalone. Perfect for special occasions.",
        ingredients: ["pasta", "abalone", "shrimp", "mussels", "garlic", "olive oil", "white wine", "salt", "pepper"]
    },

    {
        name: "Gordon Ramsay's Potato Sea Bream Dish",
        url: "https://www.youtube.com/watch?v=7UVXlMM8WzA",
        uploader: "Matsooryeong",
        tip: "A dish so delicious, it was meant for a loved one on a special day—but you'll end up wanting it all to yourself.",
        ingredients: ["Sea Bream", "Potato", "Butter", "Cream", "Shallot"]
    },
    {
        name: "Ratatouille",
        url: "https://www.youtube.com/shorts/uJZSviosysM",
        uploader: "",
        tip: "French vegetable stew. You can make it with various vegetables.",
        ingredients: ["tomato", "eggplant", "zucchini", "onion", "garlic", "olive oil", "salt", "pepper", "herbs"]
    },


    {
        name: "Currywurst",
        url: "https://www.youtube.com/watch?v=U7cujPVrudw",
        uploader: "",
        tip: "A German street food recipe that pairs perfectly with beer. Stir-fry chopped onions, add curry, ketchup, vinegar, and sugar to make the sauce. Boil the sausage, score it, grill it, and serve with fries.",
        ingredients: ["onion", "curry", "ketchup", "vinegar", "sugar", "sausage", "french fries"]
    },
    
    

    {
        name: "Flatfish Fry",
        url: "https://www.youtube.com/watch?v=KzUTBj0kK0Q",
        uploader: "Kang Leo",
        tip: "Crispy and moist fried flatfish. The ratio of batter is important.",
        ingredients: ["flatfish", "flour", "starch", "egg", "beer", "salt", "pepper"]
    },

    {
        name: "French Toast 2",
        url: "https://www.youtube.com/shorts/zGijEP_KKdY",
        uploader: "Yoo Ji Man",
        tip: "Various variations of French toast. Enjoy it with a variety of toppings.",
        ingredients: ["bread", "egg", "milk", "sugar", "butter", "berries", "maple syrup"]
    },
    {
        name: "Sashimi Carpaccio",
        url: "https://www.youtube.com/watch?v=Bhq0ApL-Un0",
        uploader: "Chef Gonggyeoksu",
        tip: "",
        ingredients: ["Yuzu Ponzu", "Sherry Vinegar", "Sugar", "Lime", "Cheongyang Chili", "Green Onion", "Minced Garlic", "Radish"]
    },
    {
        name: "Mushroom Steak Ramen",
        url: "https://www.youtube.com/watch?v=R0N3mTYFOEM",
        uploader: "Chef Gonggyeoksu",
        tip: "Easy, delicious, and beautifully plated. Legendary..",
        ingredients: ["Ojingeo Jjamppong", "Chapagetti", "Chives", "Steak", "Enoki Mushroom"]
    },
    {
        name: "Pepper Steak",
        url: "https://www.youtube.com/watch?v=WNiS9a9qTqs",
        uploader: "Kang Leo",
        tip: "A dish where the spiciness of pepper enhances the flavor of the steak. The taste may vary depending on the type of pepper used.",
        ingredients: ["Beef", "Pepper", "Olive Oil", "Salt", "Butter"]
    },
    {
        name: "Spanish Cod Dish with Bacalao & Aioli Sauce",
        url: "https://www.youtube.com/watch?v=_ioVzN12z4A",
        uploader: "Matsooryeong",
        tip: "A Spanish cod dish that perfectly suits Korean tastes.",
        ingredients: ["Tomato", "Cod Fillet", "Garlic", "Olive Oil", "Mayonnaise", "Lemon", "Paprika Powder"]
    },
    {
        name: "Pizza Collection",
        url: "https://www.youtube.com/shorts/ghX3BZrGpkg",
        uploader: "Manwon Cooking Choi Siblings",
        tip: "Can make Korean-style pizza using tortilla in the U.S.",
        ingredients: []
    },
    {
        name: "Avocado Sandwich",
        url: "https://www.youtube.com/watch?v=MreM6pizwKM",
        uploader: "Chef Gonggyeoksu",
        tip: "",
        ingredients: ["Bread", "Butter", "Avocado Oil", "Garlic", "Thyme", "Shrimp", "Egg", "Avocado", "Mayonnaise", "Sugar", "Chives"]
    },
    {
        name: "Chef Choi Hyunseok's Vongole Sujebi",
        url: "https://youtube.com/watch?v=NS8Gk8P-nAM",
        uploader: "Romantic Pig Kim Junhyun",
        tip: "The greatest chef is nature itself. Chefs only rearrange nature's ingredients.",
        ingredients: ["Garlic", "Vongole", "Sujebi (Hand-torn Dough)", "Olive Oil", "Butter", "Parsley"]
    },
    {
        name: "Mala Cream Shrimp Stew",
        url: "https://youtube.com/watch?v=NS8Gk8P-nAM",
        uploader: "Romantic Pig Kim Junhyun",
        tip: "Turning the familiar into something special.",
        ingredients: ["Prawn", "Garlic", "Mala Cream", "Parsley"]
    },
    {
        name: "Pork Jowl Confit & Spicy Apple Jam",
        url: "https://www.youtube.com/watch?v=hr2o5AqvJj0&t=729s",
        uploader: "Chef Gonggyeoksu",
        tip: "The combination of sweet apple jam and savory pork jowl is excellent.",
        ingredients: ["Pork Jowl", "Water", "Star Anise", "Garlic", "Black Peppercorn", "Salt", "Honey", "Olive Oil", "Bay Leaf", "Butter", "Apple", "Onion", "Raisin", "Ginger", "Pepper Crush", "Cinnamon Stick", "Sherry Vinegar", "Allulose", "Chives"]
    },
    {
        name: "Stir-fried Pork Jowl Ramen",
        url: "https://www.youtube.com/watch?v=JnD9ZAYIFZQ",
        uploader: "Chef Gonggyeoksu",
        tip: "Luxury rabokki style. Looks super tasty..",
        ingredients: ["Fine Chili Powder", "Paprika Powder", "Dark Soy Sauce", "Sugar", "Anchovy Fish Sauce", "Ramen", "Ramen Soup Powder", "Water", "Dried Shrimp", "Cabbage", "Onion", "Chives", "Green Onion", "Peeled Garlic", "Cheongyang Chili"]
    },
    {
        name: "White Ragu Pasta",
        url: "https://www.youtube.com/watch?v=BpnkEJ7Oyos",
        uploader: "Chef Gonggyeoksu",
        tip: "",
        ingredients: ["Minced Pork", "Minced Garlic", "Minced Celery", "Green Onion", "Black Pepper", "White Wine", "Coin Broth", "Water", "Spaghetti", "Mushroom Soup", "Unsalted Butter"]
    },
    {
        name: "Citrus Mayo Fried Shrimp",
        url: "https://youtube.com/watch?v=eMzG2QEEPQQ",
        uploader: "Chef Gonggyeoksu",
        tip: "Hold the tail when frying shrimp for better shape. Do not over-fry enoki mushrooms to avoid toughness. Adjust lime juice or mayo to taste.",
        ingredients: ["Shrimp", "Enoki Mushroom", "Potato Starch", "Egg White", "Cooking Oil", "Salt", "Citrus Marmalade", "Minced Garlic", "Mayonnaise", "Lime Juice", "Lettuce", "Lime", "Pepper"]
    },
    // {
    //     name: "Citrus Chicken Steak",
    //     url: "https://www.youtube.com/watch?v=H4TGm0L_E7",
    //     uploader: "Kang Leo",
    //     tip: "A steak where the refreshing taste of citrus balances well with the tenderness of chicken. The citrus sauce reduces the greasiness of the chicken.",
    //     ingredients: ["chicken thigh", "citrus", "onion", "garlic", "olive oil", "salt", "pepper", "rosemary"]
    // },
    {
        name: "Citrus Scallop",
        url: "https://www.youtube.com/watch?v=9Pi-qt_9YU0",
        uploader: "Kang Leo",
        tip: "A dish where the sweetness of citrus and the chewy texture of scallops harmonize well. The citrus sauce enhances the flavor of the scallops.",
        ingredients: ["scallop", "citrus", "onion", "garlic", "olive oil", "salt", "pepper", "white wine"]
    },
    {
        name: "Ragu Pasta",
        url: "https://www.youtube.com/watch?v=IizsoHay4SM",
        uploader: "Kang Leo",
        tip: "Ragu pasta with rich meat flavor and deep tomato sauce. It is characterized by its deep flavor from long simmering.",
        ingredients: ["beef", "pork", "onion", "carrot", "celery", "tomato paste", "whole tomatoes", "red wine", "pasta", "Parmigiano Reggiano cheese"]
    },
    {
        name: "Scallop and Mushroom Dish",
        url: "https://www.youtube.com/watch?v=9Pi-qt_9YU0",
        uploader: "Kang Leo",
        tip: "A dish where scallops and various mushrooms come together for a rich flavor. The taste varies depending on the types of mushrooms used.",
        ingredients: ["scallop", "king oyster mushroom", "shiitake mushroom", "button mushroom", "garlic", "olive oil", "salt", "pepper", "white wine"]
    },
    // {
    //     name: "Pork Jowl Ramen",
    //     url: "https://www.youtube.com/watch?v=JnD9ZAYIFZ",
    //     uploader: "Bab Kim Guk",
    //     tip: "A dish where the chewy texture of pork jowl and the spiciness of ramen blend perfectly. Grilled pork jowl is added to the ramen for a richer taste.",
    //     ingredients: ["ramen", "pork jowl", "onion", "green onion", "cheongyang pepper", "garlic", "chili powder", "soy sauce", "sugar"]
    // },
    {
        name: "Tomato Soup",
        url: "https://www.youtube.com/watch?v=qDF_6HLIY6o&t=316s",
        uploader: "Kang Leo",
        tip: "A soup combining the freshness of tomatoes and the flavors of various vegetables. Best enjoyed warm.",
        ingredients: ["tomato", "onion", "carrot", "celery", "garlic", "tomato paste", "olive oil", "salt", "pepper", "basil"]
    },
    {
        name: "Steak",
        url: "https://www.youtube.com/watch?v=X95NHDmx0bI",
        uploader: "Kang Leo",
        tip: "A steak that lets you enjoy the rich flavor of beef. The taste and texture vary depending on the doneness.",
        ingredients: ["beef sirloin", "olive oil", "salt", "pepper", "rosemary", "thyme"]
    },
    {
        name: "Tomato",
        url: "https://www.youtube.com/shorts/1eaz1uFwjZU",
        uploader: "Kang Leo",
        tip: "Basic information for various tomato dishes.",
        ingredients: ["tomato"]
    },
    {
        name: "Lamb Chop & Lamb Steak",
        url: "https://youtube.com/watch?v=L9vTdyl8CWI",
        uploader: "Kang Leo",
        tip: "A dish to enjoy the unique flavor of lamb. You can enjoy both lamb chops and lamb steak in one dish.",
        ingredients: ["lamb chop", "lamb steak", "olive oil", "salt", "pepper", "rosemary", "thyme", "garlic"]
    },
    {
        name: "Drinking Snack",
        url: "https://www.youtube.com/shorts/IVMx-RDDhuY",
        uploader: "Kang Leo",
        tip: "A simple and delicious snack recipe for drinks. You can make it with various ingredients.",
        ingredients: ["chicken thigh", "onion", "garlic", "chili", "soy sauce", "sugar", "chili powder"]
    },
    {
        name: "Gordon Ramsay Sandwich",
        url: "https://www.youtube.com/shorts/smIOeJRexWI",
        uploader: "Gordon Ramsay",
        tip: "A special sandwich recipe by Gordon Ramsay. Featuring fresh ingredients and a special sauce.",
        ingredients: ["baguette", "chicken breast", "bacon", "lettuce", "tomato", "mayonnaise", "mustard"]
    },
    {
        name: "Oxtail Soup",
        url: "https://www.youtube.com/watch?v=UyhrOajdpQU",
        uploader: "AhaBoojang",
        tip: "A rich and deep-flavored oxtail soup. The deep flavor comes from long simmering.",
        ingredients: ["oxtail", "radish", "green onion", "garlic", "ginger", "salt", "pepper"]
    },
    {
        name: "Boiled Oxtail",
        url: "https://www.youtube.com/watch?v=IV_fNz4ojlI",
        uploader: "Meat Lover",
        tip: "Boiled oxtail with a soft and chewy texture. Even more delicious with a special sauce.",
        ingredients: ["oxtail", "green onion", "ginger", "garlic", "soy sauce", "sugar", "cooking wine"]
    },
    {
        name: "French Toast",
        url: "https://www.youtube.com/shorts/GplGY_ExQFs",
        uploader: "Kang Leo",
        tip: "Sweet and soft French toast. Great for breakfast or brunch.",
        ingredients: ["bread", "egg", "milk", "sugar", "butter", "cinnamon powder"]
    },
    {
        name: "Currywurst (German Street Food)",
        url: "https://www.youtube.com/watch?v=U7cujPVrudw",
        uploader: "",
        tip: "A German street food recipe that pairs perfectly with beer. Stir-fry chopped onions, add curry, ketchup, vinegar, and sugar to make the sauce. Boil the sausage, score it, grill it, and serve with fries.",
        ingredients: ["onion", "curry", "ketchup", "vinegar", "sugar", "sausage", "french fries"]
    },
    {
        name: "Mazemen",
        url: "https://www.youtube.com/shorts/YoU9hquQ22o",
        uploader: "",
        tip: "Japanese-style mixed noodles. Enjoy with various toppings.",
        ingredients: ["ramen", "pork", "onion", "green onion", "garlic", "ginger", "soy sauce", "mirin", "sesame", "sesame oil"]
    },
    {
        name: "Steak Tartare",
        url: "https://www.youtube.com/shorts/YIACyktyCY8",
        uploader: "",
        tip: "A French dish made by thinly slicing raw beef. Enjoy with bread or crackers.",
        ingredients: ["beef tenderloin", "onion", "dill", "parsley", "mustard", "lemon juice", "olive oil", "salt", "pepper"]
    },
    
    {
        name: "Tantanmen",
        url: "https://www.youtube.com/watch?v=8RWnsvjxIRU",
        uploader: "",
        tip: "A spicy Japanese-style ramen topped with peanut sauce and minced meat.",
        ingredients: ["ramen", "peanut sauce", "minced meat", "green onion", "bean sprouts", "spinach", "sesame", "sesame oil"]
    },
    // {
    //     name: "Ratatouille (Kang Leo's version)",
    //     url: "https://www.youtube.com/watch?v=FdbU7-kSQ2A&t=16s",
    //     uploader: "Kang Leo",
    //     tip: "French vegetable stew. You can make it with various vegetables.",
    //     ingredients: ["tomato", "eggplant", "zucchini", "onion", "garlic", "olive oil", "salt", "pepper", "herbs"]
    // },
    // {
    //     name: "Duck with Mushrooms",
    //     url: "https://www.youtube.com/watch?v=SqJx9EgG_kQ",
    //     uploader: "Kang Leo",
    //     tip: "A dish where duck and mushrooms blend in harmony. The taste varies depending on the types of mushrooms.",
    //     ingredients: ["duck", "mushroom", "onion", "garlic", "olive oil", "salt", "pepper", "rosemary"]
    // },
    // {
    //     name: "Grilled Dokdom + Tomato Salsa",
    //     url: "https://www.youtube.com/watch?v=pK6aVCb4X3c",
    //     uploader: "Kang Leo",
    //     tip: "Grilled dokdom with a unique taste and texture. Enjoy it with tomato salsa.",
    //     ingredients: ["dokdom", "tomato", "onion", "garlic", "chili", "olive oil", "salt", "pepper", "lemon juice"]
    // },
    // {
    //     name: "Tomato Egg Garlic Pork Stir-fry",
    //     url: "https://www.youtube.com/watch?v=LOW4oXLYu64",
    //     uploader: "Kang Leo",
    //     tip: "Spicy and savory pork stir-fry. Perfect with rice.",
    //     ingredients: ["pork", "onion", "green onion", "garlic chives", "gochujang", "soy sauce", "garlic", "sugar", "sesame oil"]
    // },
    // {
    //     name: "Mediterranean Grilled Fish",
    //     url: "https://www.youtube.com/watch?v=umLY_6nOuOE",
    //     uploader: "Kang Leo",
    //     tip: "Mediterranean-style grilled fish with fresh seafood and vegetables. Healthy and delicious.",
    //     ingredients: ["salmon", "shrimp", "squid", "eggplant", "zucchini", "onion", "garlic", "olive oil", "salt", "pepper", "lemon juice"]
    // },
    {
        name: "Tomato Hangover Ramen",
        url: "https://www.youtube.com/watch?v=lIAye5iO7FY",
        uploader: "Kang Leo",
        tip: "A spicy and refreshing tomato hangover ramen. Great for hangovers.",
        ingredients: ["ramen", "tomato", "bean sprouts", "green onion", "garlic", "chili powder", "soy sauce", "sugar"]
    },
    {
        name: "Crispy Rice Fish Fry",
        url: "https://www.youtube.com/watch?v=haX9oH9ojEE",
        uploader: "",
        tip: "Fried fish coated with rice flour. Crispy and savory.",
        ingredients: ["fish", "rice flour", "flour", "egg", "salt", "pepper"]
    },
    {
        name: "Flatfish Gambas",
        url: "https://www.youtube.com/watch?v=ZU246sO-uno",
        uploader: "",
        tip: "A Spanish-style dish with flatfish and shrimp. Flavored with plenty of olive oil and garlic.",
        ingredients: ["flatfish", "shrimp", "garlic", "olive oil", "salt", "pepper", "lemon juice"]
    },
    {
        name: "Honey Cod Mayo",
        url: "https://www.youtube.com/watch?v=_ioVzN12z4A&t=429s",
        uploader: "MatSuryoung",
        tip: "A sweet and savory honey cod mayo dish. Pairs well with beer. Cod is baked with mayonnaise and honey for a rich flavor.",
        ingredients: ["cod", "mayonnaise", "honey", "lemon juice", "parsley", "salt", "pepper"]
    },

    {
        name: "Black Tomato Pasta",
        url: "https://www.youtube.com/watch?v=qJN5cu-x-B4",
        uploader: "Attacker Chef",
        tip: "A unique pasta combining black noodles and tomato sauce. Rich flavor with fresh seafood.",
        ingredients: ["black noodles", "tomato", "shrimp", "mussels", "garlic", "olive oil", "salt", "pepper", "basil"]
    },
    {
        name: "Tomato Kimchi",
        url: "https://www.youtube.com/watch?v=iaTscMN2FNE",
        uploader: "",
        tip: "A unique kimchi made with tomatoes. The freshness of tomatoes and the spiciness of kimchi blend harmoniously.",
        ingredients: ["tomato", "cabbage", "onion", "garlic", "chili powder", "fish sauce", "sugar", "salt"]
    },
    {
        name: "Pork Jowl",
        url: "https://www.youtube.com/shorts/X0ijVGo0v4Q",
        uploader: "Bab Kim Guk",
        tip: "A dish featuring the chewy and savory flavor of pork jowl. Can be cooked in various ways.",
        ingredients: ["pork jowl", "salt", "pepper", "herbs"]
    },
    {
        name: "Butter Pasta",
        url: "https://www.youtube.com/watch?v=iKaLAnPZGyM",
        uploader: "MatDongmu",
        tip: "A pasta rich in the flavor of butter. Simple yet sophisticated.",
        ingredients: ["pasta", "butter", "garlic", "Parmigiano Reggiano cheese", "salt", "pepper"]
    },
    {
        name: "Pork Steak",
        url: "https://www.youtube.com/watch?v=CVyniN--dDE",
        uploader: "Kang Leo",
        tip: "Enjoy the rich taste of pork steak. The flavor varies depending on the degree of doneness.",
        ingredients: ["pork shoulder", "olive oil", "salt", "pepper", "rosemary", "thyme", "garlic"]
    },
    {
        name: "Clafoutis = Cherry Dessert",
        url: "https://www.youtube.com/watch?v=7-NXoPYmO3c",
        uploader: "Kang Leo",
        tip: "No oven needed.",
        ingredients: ["cherry", "flour", "egg", "milk", "sugar", "butter", "almond slices"]
    },
    {
        name: "Eggs Benedict",
        url: "https://www.youtube.com/watch?v=3UKlryG_riE",
        uploader: "Kang Leo",
        tip: "A classic brunch menu with soft poached eggs, English muffins, and hollandaise sauce.",
        ingredients: ["English muffin", "egg", "bacon", "spinach", "hollandaise sauce"]
    },
    {
        name: "Real Man's Dessert",
        url: "https://www.youtube.com/watch?v=bujZkYFGzGU",
        uploader: "",
        tip: "Korean snacks. Can be used to make a variety of desserts like ice cream. Could have global potential!",
        ingredients: ["Korean snacks", "ice cream", "fruit", "chocolate"]
    },
    // {
    //     name: "Butter & Pudding Bread",
    //     url: "https://www.youtube.com/watch?v=R2ysLQYxqt",
    //     uploader: "Kang Leo",
    //     tip: "Bread with a soft combination of butter and pudding. Tastes even better when warm.",
    //     ingredients: ["bread", "butter", "pudding", "egg", "milk", "sugar", "cinnamon powder"]
    // },
    {
        name: "Salmon Ceviche",
        url: "https://www.youtube.com/watch?v=Yk8QtDuFo9A",
        uploader: "MatSuryoung",
        tip: "Super easy.",
        ingredients: ["salmon", "onion", "cilantro", "lime juice", "olive oil", "salt", "pepper"]
    },
    {
        name: "Butterfish Ceviche",
        url: "https://www.youtube.com/watch?v=uyGI3hLpzMc",
        uploader: "Kang Leo",
        tip: "Butterfish has white flesh and almost no fishy smell.",
        ingredients: ["butterfish", "onion", "cilantro", "lime juice", "olive oil", "salt", "pepper"]
    },
    {
        name: "Salmon Crispy Steak",
        url: "https://www.youtube.com/watch?v=uMWIQXm-FpY",
        uploader: "Attacker Chef",
        tip: "Looks delicious",
        ingredients: ["salmon", "flour", "egg", "breadcrumbs", "olive oil", "salt", "pepper", "sauce"]
    },
    {
        name: "Kimchi Pasta",
        url: "https://www.youtube.com/watch?v=6epy51dKxaQ",
        uploader: "Attacker Chef",
        tip: "Looks insanely good. Green onion heart shape service ><",
        ingredients: ["pasta noodles", "kimchi", "pork", "onion", "garlic", "chili powder", "soy sauce", "sesame oil"]
    },
    {
        name: "Spanish Omelette",
        url: "https://www.youtube.com/watch?v=xssPQ7fS7PI",
        uploader: "Kang Leo",
        tip: "A hearty and healthy Spanish-style omelet with various vegetables, perfect for a meal.",
        ingredients: ["egg", "potato", "onion", "bell pepper", "olive oil", "salt", "pepper"]
    },
    {
        name: "Tomato Burrata Cheese Salad",
        url: "https://www.youtube.com/shorts/nCHzy54eGWs",
        uploader: "",
        tip: "A salad that harmonizes fresh tomatoes with soft burrata cheese.",
        ingredients: ["tomato", "burrata cheese", "basil", "olive oil", "salt", "pepper"]
    },
    {
        name: "Kiwi Carpaccio",
        url: "https://www.youtube.com/shorts/ohDIcCXIteY",
        uploader: "Gongshef",
        tip: "Super super super super easy ㄷㄷ",
        ingredients: ["kiwi", "olive oil", "lemon juice", "salt", "pepper", "arugula"]
    },
    {
        name: "The Easiest Gourmet Salmon Confit Dish",
        url: "https://www.youtube.com/watch?v=2S393ovRVrM",
        uploader: "Kang Leo",
        tip: "Tasty, extremely simple and easy.",
        ingredients: ["salmon", "olive oil", "herbs", "salt", "pepper"]
    },
    {
        name: "Lamb Steak",
        url: "https://www.youtube.com/watch?v=L9vTdyl8CWI",
        uploader: "Kang Leo",
        tip: "Looks super easy and delicious..",
        ingredients: ["lamb", "olive oil", "rosemary", "thyme", "garlic", "salt", "pepper"]
    },
    {
        name: "Salmon Confit and Brunch",
        url: "https://www.youtube.com/watch?v=0N9-mpDBzxM",
        uploader: "",
        tip: "Simple and looks delicious",
        ingredients: ["salmon", "olive oil", "herbs", "egg", "bread", "vegetables"]
    },
    {
        name: "Tomato Kimchi Pork Stew",
        url: "https://www.youtube.com/watch?v=jyfdFs_894c",
        uploader: "Attacker Chef",
        tip: "Simple and looks super tasty..",
        ingredients: ["tomato", "kimchi", "pork", "onion", "garlic", "chili powder", "soy sauce", "sugar"]
    },
    {
        name: "Chicken Leg Steak",
        url: "https://www.youtube.com/watch?v=P1ID52sPl_c",
        uploader: "Masuryeong",
        tip: "Simple and looks delicious",
        ingredients: ["chicken leg", "olive oil", "salt", "pepper", "herbs"]
    },
    {
        name: "Whole Canned Tomato Pasta Tutorial",
        url: "https://youtube.com/watch?v=FnOxV_6EXgs",
        uploader: "Masuryeong",
        tip: "A tutorial video for pasta using whole canned tomatoes.",
        ingredients: ["whole canned tomato", "pasta noodles", "garlic", "olive oil", "salt", "pepper"]
    },
    {
        name: "Salmon Mixed Rice",
        url: "https://www.youtube.com/watch?v=U2PyiSXAlrs",
        uploader: "Choi Kang-rok",
        tip: "Super super super easy",
        ingredients: ["salmon", "rice", "soy sauce", "sesame oil", "seaweed"]
    },
    {
        name: "Cabbage Steak",
        url: "https://www.youtube.com/watch?v=MnC46BncM18",
        uploader: "Chef Ho-yoon",
        tip: "Extremely delicious, simple, healthy-looking, and luxurious",
        ingredients: ["cabbage", "butter", "garlic", "olive oil", "salt", "pepper", "cheese"]
    },
    {
        name: "Cream Lobster",
        url: "https://www.youtube.com/shorts/IVPbxQHr9zc",
        uploader: "Attacker Chef",
        tip: "Enjoy a luxurious lobster dish with cream sauce.",
        ingredients: ["lobster", "fresh cream", "garlic", "onion", "white wine", "parsley"]
    },
    {
        name: "Creamy Meat and Mushroom Stew",
        url: "https://www.youtube.com/watch?v=FjArRPBN7iQ&t=10s",
        uploader: "Kang Leo",
        tip: "A stew with a soft cream sauce combined with meat and mushroom flavors.",
        ingredients: ["beef", "mushroom", "onion", "garlic", "fresh cream", "white wine", "parsley"]
    },
    // {
    //     name: "Beef, Mushroom, Arugula, and Onion Salad",
    //     url: "https://www.youtube.com/watch?v=SqJx9EgG_kQ&t=46s",
    //     uploader: "Kang Leo",
    //     tip: "A salad where various mushrooms, arugula, onion, and beef can be enjoyed together.",
    //     ingredients: ["beef", "mushroom", "arugula", "onion", "olive oil", "balsamic vinegar"]
    // },
    {
        name: "Spinach Pasta",
        url: "https://www.youtube.com/shorts/zNfd-IOzulU",
        uploader: "",
        tip: "A dish where the freshness of spinach harmonizes with pasta.",
        ingredients: ["pasta noodles", "spinach", "garlic", "olive oil", "parmesan cheese"]
    },
    {
        name: "Spinach Tomato Onion Greek Yogurt Pizza",
        url: "https://youtube.com/shorts/rN1N-Fj3n9Y",
        uploader: "",
        tip: "A special pizza recipe using healthy ingredients.",
        ingredients: ["tortilla", "spinach", "tomato", "onion", "Greek yogurt", "cheese"]
    },
    {
        name: "Vongole Sujebi & Mala Cream Shrimp",
        url: "https://www.youtube.com/watch?v=NS8Gk8P-nAM",
        uploader: "",
        tip: "Super easy and delicious recipe. The best chef is nature itself. We just rearrange it. Making the familiar special.",
        ingredients: ["hand-torn noodles", "clams", "shrimp", "garlic", "olive oil", "mala sauce", "fresh cream"]
    },
    {
        name: "American Style Japanese Ramen",
        url: "https://www.youtube.com/watch?v=0O03sStIUVk&list=LL&index=1",
        uploader: "",
        tip: "Super simple and looks delicious",
        ingredients: ["No ingredient information"]
    },
    {
        name: "Fruit Cocktail",
        url: "https://www.youtube.com/shorts/6O6jBeucKko",
        uploader: "Acoustic Drink",
        tip: "A refreshing cocktail recipe using various fruits.",
        ingredients: ["fruit", "soda", "alcohol"]
    }
    

];

export default menuData_en;
